...
* Plugin URI: https://github.com/yysofiyan
...
function spp_save_social_links_fields($user_id) {
    ...
    $fields = ['social_garuda', 'social_orcid', 'social_scholar', 'social_scopus', 'social_publon', 'social_wos', 'social_github'];
    ...
    if (is_singular('post')) {
        ...
        foreach ($social_profiles as $platform => $data) {
            if (!empty($data['link'])) {
                ...
                $social_links_html .= '<img src="' . esc_url($data['icon']) . '" alt="' . esc_attr($platform) . '" width="20" height="20" />';
                $social_links_html .= '</a> ';
            }
        }
...
function spp_enqueue_styles() {
    wp_enqueue_style('spp-author-box-style', plugins_url('style.css', __FILE__), [], '1.1');
}
